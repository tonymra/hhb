...
var SupportDiv = document.getElementById('errormsgeditclasses');
	...
	$(".btn-edit-classes").click(function(e) {
		...
		$.ajax({
			url: base_url + "admin/classes/edit_pro",
			type: 'POST',
			data: { name: name,
			...
			success: function(data) {
				...
				if(data.error == 291) {
					$(".alert-error-editclasses").removeClass("hidden");
					$(".alert-error-editclasses").html('<span style="color: #C00"><i class="fa fa-warning (alias)"></i>&nbsp;' + data.error_msg + '</span>').fadeIn();
					...
					window.location = base_url + "admin/classes" + "?02Gy4bDO2QBpxEpQpZOmN2jr9gFwO6lqWAr6lVLp";
				}

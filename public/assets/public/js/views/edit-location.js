...
var SupportDiv = document.getElementById('errormsgeditlocation');
	...
	$(".btn-edit-location").click(function(e) {
		...
		$.ajax({
			url: base_url + "admin/locations/edit_pro",
			type: 'POST',
			data: {name: name,
			       address: address,
				   id: id,
			...
			success: function(data) {
				...
				if(data.error == 291) {
					$(".alert-error-editlocation").removeClass("hidden");
					$(".alert-error-editlocation").html('<span style="color: #C00"><i class="fa fa-warning (alias)"></i>&nbsp;' + data.error_msg + '</span>').fadeIn();
					...
					window.location = base_url + "admin/locations" + "?02Gy4bDO2QBpxEpQpZOmN2jr9gFwO6lqWAr6lVLp";
				}
